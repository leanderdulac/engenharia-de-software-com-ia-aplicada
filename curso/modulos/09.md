...
## Governança de dados / preparação de datasets

O ciclo Amplitude já exige schema, dedup, PII e DPA no JSON da pasta 01. Antes de C09.2 / C09.3, feche este checklist (labs da pasta 02 + manifesto). Ideias de manifesto, provenance e split por entidade vêm do curso companheiro [lucianoon/llm-course](https://github.com/lucianoon/llm-course) (Apache-2.0; fork [leanderdulac/curso-de-engenharia-de-IA](https://github.com/leanderdulac/curso-de-engenharia-de-IA)) — texto isolado em [`referencia/llm-course/docs/GOVERNANCA-DE-DADOS.md`](../../referencia/llm-course/docs/GOVERNANCA-DE-DADOS.md).

- [ ] **PII:** `pii-scrubbing-gate-tool` (JS ou Python) passou; revisão humana anotada. Scanner automático é cobertura baixa (e-mail/CPF/telefone), não garantia.
- [ ] **Provenance:** origem, licença, finalidade, responsável — inclusive dataset sintético.
- [ ] **Manifesto:** checksum, bytes, transformações (OCR, MinHash, split). Opcional: [`referencia/llm-course/tools/auditar_dataset.py`](../../referencia/llm-course/tools/auditar_dataset.py) (`PYTHONPATH` no README do vendor).
- [ ] **Splits:** por entidade ou tempo (segurado, apólice, documento). Não embaralhe a mesma pessoa em treino e teste.
- [ ] **Leakage:** Auto vs Saúde discriminados (`linha`); conjuntos `amplitude-auto-only-120` / `amplitude-saude-only-80` existem para pegar regressão cruzada.
- [ ] Brutos e `adapters.safetensors` **fora** do Git; logs sem colar dado pessoal.

Não reescreva as tools TS/Python da Amplitude. O vendor só audita e documenta.

## Referência Python (llm-course)

Opcional, **depois** (ou em paralelo curto) dos labs Amplitude. Código Apache só em `referencia/llm-course/`.

- LoRA / NF4 do zero (CPU): [`referencia/llm-course/modulo-06-lora/`](../../referencia/llm-course/modulo-06-lora/) — casa com 9.4. Execução completa pede corpus MiniGPT do curso B; `lab_mlx.py` é Mac + dados SFT de B.
- Governança / manifesto: [`tools/auditar_dataset.py`](../../referencia/llm-course/tools/auditar_dataset.py) — casa com 9.2. Roda neste recorte.
- Protocolo de eval (hipótese, baseline, IC, erro): [`docs/PROTOCOLO-DE-AVALIACAO.md`](../../referencia/llm-course/docs/PROTOCOLO-DE-AVALIACAO.md) — casa com 9.5 e com [avaliacao.md](../avaliacao.md).
- Empacotamento: [`referencia/llm-course/README.md`](../../referencia/llm-course/README.md). Crédito: [`curso/ATRIBUICOES-llm-course.md`](../ATRIBUICOES-llm-course.md).
